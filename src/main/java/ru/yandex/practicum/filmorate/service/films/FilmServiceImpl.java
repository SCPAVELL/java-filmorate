...
public class FilmServiceImpl implements FilmService {
	...
	public List<Film> search(String query, List<String> filters) {
		if (filters.isEmpty() || !(filters.contains("title") || filters.contains("director")) || filters.size() > 2) {
			throw new ValidationException("Фильтр поиска задан некорректно: количество параметров: " + filters.size()
					+ " - Должен быть 1 или 2; В запросе присутствуют фильтры: " + filters
	...
	private void checkDirectors(Film film) {
		if (film.getDirectors() != null) {
			List<Integer> ids = film.getDirectors().stream().map(Director::getId).collect(Collectors.toList());
			...
			film.setDirectors(
					directors.stream().sorted(Comparator.comparing(Director::getId)).collect(Collectors.toList()));
		}
	}
