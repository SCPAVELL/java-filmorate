...
public class FilmDbStorage implements FilmStorage {
	...
	private static final String SQL_GET_DIRECTOR_FILMS_SORTED_BY_LIKES = "select f.*, m.* from films as f "
			+ "join mpa_ratings as m on m.id = f.rating_id " + "left join likers as l on l.film_id = f.id "
			+ "join films_directors as fd on fd.film_id = f.id " + "where fd.director_id = ? " + "group by f.id "
			+ "order by count(l.film_id) desc";
	private static final String SQL_GET_DIRECTOR_FILMS_SORTED_BY_YEARS = "select f.*, m.* from films as f "
			+ "join mpa_ratings as m on m.id = f.rating_id " + "join films_directors as fd on fd.film_id = f.id "
	...
	private static final String FIND_MOST_POPULAR_FILMS_BY_NAME_SQL = "SELECT f.* " + "FROM films AS f "
			+ "LEFT JOIN likers AS l ON l.film_id = f.id " + "WHERE LOWER(f.title) LIKE LOWER(?) " + "GROUP BY f.id "
			+ "ORDER BY COUNT(l.film_id) DESC";
	private static final String FIND_MOST_POPULAR_FILMS_BY_DIRECTOR_SQL = "SELECT f.* " + "FROM films AS f "
			+ "LEFT JOIN likers AS l ON l.film_id = f.id " + "LEFT JOIN films_directors AS fd ON fd.film_id=f.id "
			+ "LEFT JOIN directors AS d ON fd.director_id=d.id " + "WHERE LOWER(d.name) LIKE LOWER(?)"
			+ "GROUP BY f.id " + "ORDER BY COUNT(l.film_id) DESC";
	private static final String FIND_MOST_POPULAR_FILMS_BY_NAME_AND_DIRECTOR_SQL = "SELECT f.* " + "FROM films AS f "
			+ "LEFT JOIN likers AS l ON l.film_id = f.id " + "LEFT JOIN films_directors AS fd ON fd.film_id=f.id "
			+ "LEFT JOIN directors AS d ON fd.director_id=d.id "
			+ "WHERE (LOWER(d.name) LIKE LOWER(?) OR LOWER(f.title) LIKE LOWER(?)) " + "GROUP BY f.id "
			+ "ORDER BY COUNT(l.film_id) DESC";
	private static final String SQL_GET_COMMON = "SELECT f.*, COUNT(l.film_id) AS like_count\n" + "FROM films AS f "
			+ "JOIN likers AS l ON f.id = l.film_id " + "WHERE l.liker_id = ? " + "AND l.film_id IN ("
			+ "    SELECT l.film_id " + "    FROM likers l " + "    WHERE l.liker_id = ?) " + "GROUP BY f.id "
	...
	public Film add(Film film) {
		...
		for (Genre genre : film.getGenres()) {
			genreStorage.getById(genre.getId())
					.orElseThrow(() -> new ValidationException("Неправильно набран рейтинг!"));
		}
	...
	public Film update(Film film) {
		...
		for (Genre genre : genres) {
			Genre g = genreStorage.getById(genre.getId())
					.orElseThrow(() -> new ValidationException("Неправильно набран рейтинг!"));
	...
	public Optional<Film> getById(Long id) {
		...
		if (rs.next()) {
			Film film = new Film.Builder().id(rs.getLong("id")).name(rs.getString("title"))
					.description(rs.getString("description")).releaseDate(rs.getDate("release_date").toLocalDate())
					.duration(rs.getInt("duration")).mpa(ratingStorage.getByFilmId(id).orElse(null))
					.genres(genreStorage.getByFilmId(id)).directors(directorStorage.getAllFilmDirectors(id)).build();
	...
	public List<Film> search(String query, List<String> filters) {
		...
		if (filters.contains("director") && filters.contains("title")) {
			return jdbcTemplate.query(FIND_MOST_POPULAR_FILMS_BY_NAME_AND_DIRECTOR_SQL, mapper, updatedQuery,
					updatedQuery);
	...
	@Override
	public List<Film> getMostPopularFilms(Long count, @Nullable Integer genreId, @Nullable Integer year) {
		if (genreId != null) {
	...
	private String getSqlQueryForPopularFilms(Long count, @Nullable Integer genreId, @Nullable Integer year) {
