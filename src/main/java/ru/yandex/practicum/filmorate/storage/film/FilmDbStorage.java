...
public class FilmDbStorage implements FilmStorage {
	...
	public Film add(Film film) throws ResponseStatusException {
		...
		String sqlQuery2 = "INSERT INTO director_films (film_id, director_id) " + " VALUES (?, ?)";
		if (film.getDirectors() != null) {
			for (Director director : film.getDirectors()) {
				jdbcTemplate.update(sqlQuery2, filmId, director.getId());
			}
		}
	...
	public List<Film> getCommonFilms(Integer userId, Integer friendId) {
		...
		String sqlQuery = "SELECT f.*, m.mpa_name " + "FROM film AS f " + "LEFT JOIN mpa AS m ON f.mpa = m.mpa_id "
				+ "LEFT JOIN likes AS l on f.film_id = l.film_id "
				+ "WHERE f.film_id IN (SELECT f.film_id FROM film AS f "
				+ "LEFT JOIN likes lu on lu.film_id = f.film_id " + "LEFT JOIN likes lf on lf.film_id = f.film_id "
				+ "WHERE lu.person_id = ? AND lf.person_id = ?) " + "GROUP BY f.film_id "
	...
	public List<Film> getTopFilms(Integer count, Integer genreId, Integer year) {
		...
		if (genreId != null && year != null) {
			...
			String sqlQuery = "SELECT f.*, m.mpa_name FROM film AS f " + "LEFT JOIN mpa AS m ON f.mpa = m.mpa_id "
					+ "LEFT JOIN genre_films AS gf ON f.film_id = gf.film_id "
					+ "LEFT JOIN genre AS g ON gf.genre_id = g.genre_id "
					+ "LEFT JOIN likes AS l ON f.film_id = l.film_id " + "WHERE g.genre_id = ? "
					+ "GROUP BY f.film_id ORDER BY COUNT(l.person_id) DESC LIMIT ?";
	...
	public List<Film> getSortedDirectorFilms(Integer directorId, String sortBy) {
		...
		case "likes":
			sqlQuery = "SELECT f.*, m.mpa_name FROM film AS f " + "LEFT JOIN mpa AS m ON f.mpa = m.mpa_id "
					+ "LEFT JOIN director_films AS df ON f.film_id = df.film_id "
					+ "LEFT JOIN director AS d ON df.director_id = d.director_id "
					+ "LEFT JOIN likes AS l ON f.film_id = l.film_id " + "WHERE d.director_id = ? GROUP BY f.film_id "
					+ "ORDER BY COUNT(l.person_id) DESC";
			return jdbcTemplate.query(sqlQuery, this::makeFilm, directorId);
		default:
	...
	public void addLike(Integer userId, Integer filmId) throws ResponseStatusException {
		...
		if (!dbContainsFilm(filmId)) {
			String message = "Ошибка запроса добавления лайка фильму." + " Невозможно поставить лайк фильму с id= "
					+ filmId + " которого не существует.";
			throw new ResponseStatusException(HttpStatus.NOT_FOUND, message);
		}
	...
	public void deleteLike(Integer userId, Integer filmId) {
		if (!dbContainsUser(userId)) {
			String message = "Ошибка запроса удаления лайка" + " Невозможно удалить лайк от пользователя с id= "
					+ userId + " которого не существует.";
			throw new ResponseStatusException(HttpStatus.NOT_FOUND, message);
		}
		if (!dbContainsFilm(filmId)) {
			String message = "Ошибка запроса удаления лайка" + " Невозможно удалить лайк с фильма с id= " + filmId
	...
	public List<Film> getFilmByTitle(String filmName) {
		String sqlQuery = "SELECT f.*, m.mpa_name, l.person_id " + "FROM film AS f "
				+ "JOIN mpa AS m on f.mpa = m.mpa_id " + "LEFT JOIN likes l on f.film_id = l.film_id "
				+ "WHERE LOWER(f.name) LIKE ? " + "GROUP BY f.film_id ORDER BY COUNT(l.person_id) DESC";
	...
	public List<Film> getFilmByDirector(String directorName) {
		String sqlQuery = "SELECT f.*, m.mpa_name, d.director_name " + "FROM film AS f "
				+ "LEFT JOIN mpa AS M on f.mpa = m.mpa_id " + "LEFT JOIN likes l on f.film_id = l.film_id "
				+ "LEFT JOIN director_films AS df ON f.film_id = df.film_id "
				+ "LEFT JOIN director AS d ON d.director_id = df.director_id " + "WHERE LOWER(d.director_name) LIKE ? "
	...
	public List<Film> getFilmByTitleDirector(String titleDirector) {
		String sqlQuery = "SELECT F.*, M.mpa_name, D.DIRECTOR_NAME FROM film AS F "
				+ "LEFT JOIN MPA AS M on F.MPA = M.MPA_ID " + "LEFT JOIN LIKES AS L on F.FILM_ID = L.FILM_ID "
				+ "LEFT JOIN director_films AS df ON f.film_id = df.film_id "
