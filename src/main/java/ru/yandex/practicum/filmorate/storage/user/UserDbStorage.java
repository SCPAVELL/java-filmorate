...
public class UserDbStorage implements UserStorage {
	...
	public void addFriend(Integer userId, Integer friendId) throws ResponseStatusException {
		...
		try {
			...
			String message = "Ошибка запроса добавления в друзья." + " Невозможно добавиться в друзья самому к себе";
			throw new ResponseStatusException(HttpStatus.BAD_REQUEST, message);
		}
	}
	...
	public List<User> getCommonFriends(Integer userId, Integer friendId) throws ResponseStatusException {
		...
		String sqlQuery = "SELECT * " + "FROM person " + "WHERE person_id IN " + "(SELECT * FROM (SELECT  addressee_id "
				+ "FROM FRIEND_REQUEST "
	...
	private User makeUser(ResultSet resultSet, int rowNum) throws SQLException {
		User user = User.builder().id(resultSet.getInt("person_id")).email(resultSet.getString("email"))
	...
	private Feed makeFeed(ResultSet rs, int rowNum) throws SQLException {
		return Feed.builder().userId(rs.getInt("person_id")).eventType(rs.getString("event_type"))
				.operation(rs.getString("operation")).eventId(rs.getInt("event_id")).entityId(rs.getInt("entity_id"))
				.timestamp(rs.getTimestamp("time_stamp")).build();
	}

	private boolean dbContainsUser(User user) {
		String sqlQuery = "SELECT * FROM person WHERE email = ? AND login = ? AND name = ? AND birthday = ?";
		try {
			jdbcTemplate.queryForObject(sqlQuery, this::makeUser, user.getEmail(), user.getLogin(), user.getName(),
					user.getBirthday());
			return true;
