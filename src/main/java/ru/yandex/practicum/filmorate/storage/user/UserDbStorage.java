...
public class UserDbStorage implements UserStorage {
	...
	private User addWithSpecifiedId(User user) {
		jdbcTemplate.update(INSERT_USER_WITH_ID_SQL, user.getId(), user.getEmail(), user.getLogin(), selectName(user),
	...
	public Optional<User> getById(Long id) {
		...
		if (rs.next()) {
			User user = new User.Builder().id(id).email(rs.getString(2)).login(rs.getString(3)).name(rs.getString(4))
	...
	public List<Long> getRecommendations(Long id) {
		...
		// Заполняем likersToFilmsMap: Ключ - id лайкнувшего, Значение - список
		// лайкнутых фильмов
		...
		/*
		 * Перебираем остальных юзеров, считаем количество сходств и одновременно
		 * сохраняем нелайкнутые нашим юзером фильмы, потом проверяем, если счетчик
		 * больше максимального количества схождений, значит, надо сохранить список
		 * нелайкнутых фильмов, так как это потенциальный результат
		 */
