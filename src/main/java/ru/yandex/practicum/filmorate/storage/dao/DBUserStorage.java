...
public class DBUserStorage implements UserStorage {
	...
	public User updateUser(User user) {
		String sqlUser = "update USERS set " + "EMAIL = ?, LOGIN = ?, NAME = ?, BIRTHDAY = ? " + "where USERID = ?";
	...
	private User makeUser(ResultSet resultSet) throws SQLException {
		...
		return new User(userId, resultSet.getString("Email"), resultSet.getString("Login"), resultSet.getString("Name"),
				Objects.requireNonNull(resultSet.getDate("BirthDay")).toLocalDate(), getUserFriends(userId));
	}
	...
	public boolean addFriend(int userId, int friendId) {
		...
		String sqlGetReversFriend = "select * from FRIENDSHIP " + "where USERID = ? and FRIENDID = ?";
		SqlRowSet sqlRowSet = jdbcTemplate.queryForRowSet(sqlGetReversFriend, friendId, userId);
		friendAccepted = sqlRowSet.next();
		String sqlSetFriend = "insert into FRIENDSHIP (USERID, FRIENDID, STATUS) " + "VALUES (?,?,?)";
		jdbcTemplate.update(sqlSetFriend, userId, friendId, friendAccepted);
		if (friendAccepted) {
			String sqlSetStatus = "update FRIENDSHIP set STATUS = true " + "where USERID = ? and FRIENDID = ?";
	...
	public boolean deleteFriend(int userId, int friendId) {
		...
		String sqlSetStatus = "update FRIENDSHIP set STATUS = false " + "where USERID = ? and FRIENDID = ?";
		jdbcTemplate.update(sqlSetStatus, friendId, userId);
		return true;
	}
